...
struct DashboardView: View {
    ...
    private var scanWarningBanner: some View {
        ...
        return VStack(alignment: .leading, spacing: 6) {
            ...
            Text(isID ? "macOS membatasi akses pada beberapa folder sistem. SweepMyMac melewatinya dengan aman dan tetap menampilkan hasil dari area yang dapat diakses." : "macOS restricts access to certain system folders. SweepMyMac safely skips them and shows results from accessible areas.")
                .font(.caption)
                .foregroundColor(Color.mcOnSurfaceVariant)
        }
        .padding(12)
...
struct RecommendationExplanationView: View {
    ...
    var body: some View {
        ...
        return VStack(spacing: 16) {
            ...
            let introText = isID ? "SweepMyMac mengidentifikasi \(count) item dengan tingkat keamanan tinggi sebesar \(ByteFormatter.string(from: bytes))." : "SweepMyMac identified \(count) high-safety items totaling \(ByteFormatter.string(from: bytes))."
            Text(introText)
                .font(.body)
                .foregroundColor(Color.mcOnSurfaceVariant)
