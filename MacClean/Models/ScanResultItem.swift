...
struct ScanResultItem: Identifiable, Equatable, Hashable, Sendable {
    ...
    func whyDialogTitle(for language: AppLanguage) -> String {
        ...
        case .safeToClean:
            return isID ? "Mengapa MacClean dapat membersihkan ini?" : "Why can MacClean clean this?"
        case .needsReview:
    ...
    func consequenceExplanationText(for language: AppLanguage) -> String {
        ...
        switch confidenceTier {
        case .safeToClean:
            return isID ? "Aman memindahkan item ini ke Tempat Sampah. MacClean tidak menghapus berkas secara permanen, dan Anda dapat memulihkannya dari Tempat Sampah jika diperlukan." : "Moving this item to Trash is safe. MacClean does not permanently delete files, and you can restore it from Trash if needed."
        case .needsReview:
            if category == .largeFiles {
        ...
        case .protected:
            return isID ? "MacClean menjaga data ini tetap terlindungi agar Anda tidak kehilangan arsip penting, keluaran build, atau status sistem yang kritis." : "MacClean keeps this data protected to avoid losing important archives, build outputs, or critical system state."
        }
    }
